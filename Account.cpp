...
Account::Account(const  string& name, const  string& sername, const  string& bDate):_name(name),
	_sername(sername),_birthdate(bDate){
		return;
}
Account::Account(const Account& ac):_name(ac.name()),
	_sername(ac.sername()),_birthdate(ac.birthdate()),_cards(ac.cards()){
		return;

}

Account::Account():_name(""),_sername(""),_birthdate(""){
	return;
}
